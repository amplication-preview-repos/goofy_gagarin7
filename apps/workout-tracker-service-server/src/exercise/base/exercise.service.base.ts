/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { PrismaService } from "../../prisma/prisma.service";

import {
  Prisma,
  Exercise as PrismaExercise,
  Progress as PrismaProgress,
  Workout as PrismaWorkout,
} from "@prisma/client";

export class ExerciseServiceBase {
  constructor(protected readonly prisma: PrismaService) {}

  async count(args: Omit<Prisma.ExerciseCountArgs, "select">): Promise<number> {
    return this.prisma.exercise.count(args);
  }

  async exercises(
    args: Prisma.ExerciseFindManyArgs
  ): Promise<PrismaExercise[]> {
    return this.prisma.exercise.findMany(args);
  }
  async exercise(
    args: Prisma.ExerciseFindUniqueArgs
  ): Promise<PrismaExercise | null> {
    return this.prisma.exercise.findUnique(args);
  }
  async createExercise(
    args: Prisma.ExerciseCreateArgs
  ): Promise<PrismaExercise> {
    return this.prisma.exercise.create(args);
  }
  async updateExercise(
    args: Prisma.ExerciseUpdateArgs
  ): Promise<PrismaExercise> {
    return this.prisma.exercise.update(args);
  }
  async deleteExercise(
    args: Prisma.ExerciseDeleteArgs
  ): Promise<PrismaExercise> {
    return this.prisma.exercise.delete(args);
  }

  async findProgresses(
    parentId: string,
    args: Prisma.ProgressFindManyArgs
  ): Promise<PrismaProgress[]> {
    return this.prisma.exercise
      .findUniqueOrThrow({
        where: { id: parentId },
      })
      .progresses(args);
  }

  async getWorkout(parentId: string): Promise<PrismaWorkout | null> {
    return this.prisma.exercise
      .findUnique({
        where: { id: parentId },
      })
      .workout();
  }
}
